            ...
            foreach ($rows as $row) {
                ...
                foreach ($columns as $column) {
                    ...
                    if ($calculated)
                        ...
                        $datablock = DataBlocks::getValueByTagsArray([$row, $column]);
                    //\app\libraries\datablocks\DataBlock::getByTagsArray(array($row, $column));
                    
                    if (isset($datablock)) {
                        ?> <input type="text" class="form-control input-sm"
                                  datablock="<?php echo $calculated ? $datablock->get_id() : $datablock->id ?>"
                                  value="<?php echo $calculated ? $datablock->getProccessedValue() : $datablock->value; ?>"> <?php
                    }
                    echo '</td>';
                    self::$progress += 1;
            ...
            foreach ($parent->get_children()->getAsArray() as $child) {
                ...
                foreach ($columns as $column) {
                    ...
                    if ($calculated)
                        ...
                        $datablock = DataBlocks::getValueByTagsArray([$child, $column]);
