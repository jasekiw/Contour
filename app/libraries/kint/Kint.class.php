...
function getExceptionTraceAsString($exception) {
	...
	{
		echo 'FILE: ' . $exception->getFile() . ' - Error: ' . $message . ' ' . " - Line: " . $exception->getLine();
		return;
	}


	echo  call_user_func( array( 'Kint_Decorators_Rich', 'init' ) );
	...
	<div class="kint trace">
		<dl>
			<dt class="kint-parent kint-show"><span class="kint-popup-trigger" title="Open in new window"></span>
				<nav>&nbsp;&nbsp;&nbsp;FILE: ' . $exception->getFile() . ' | With Message: ' . $message . ' ' . " | On Line: " . $exception->getLine() .'</nav>
				<dfn></dfn> <var></var>
			</dt>
			<dd>
            	<pre class="trace_content">
            		<table>
            			<tbody>';
	$count = 0;
...
class Kint
{
	...
	public static function trace( $trace = null )
	{
		...
		return  isset( $trace ) ? printf(getExceptionTraceAsString($trace)) : self::dump(debug_backtrace( true )) ;
	}
